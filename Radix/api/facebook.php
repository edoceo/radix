...
{
    const USER_AGENT = 'Edoceo Radix Facebook v2012.12';
    ...
    */                                                                                                               
    ...
    /**
    */
    public function killCookie($host)
    {
        $key = 'fbs_' . self::$_app_id;
        setcookie($key,false,$_SERVER['REQUEST_TIME']-86400,'/',$host);
    }
    /**
        Designed to Get Information from the Cookie
        @return true|false
    */
    public function loadCookie()
    {
        ...
        $buf = trim($_COOKIE['fbs_' . self::$_app_id],'\\" ');
        $arg = null;
        parse_str($buf,$arg);

        //if ($arg['expires'] > $_SERVER['REQUEST_TIME']) {
        //    return false;
        //}
        ...
        $chk = md5($chk . self::$_secret);
        if ($chk != $sig) {
            return $_ret;
        }
        $this->_access_token = $arg['access_token'];
        $this->_cookie = $arg;
        $_ret = true;
        return $_ret;
    }
